...
router.get('/editpost/:id', authenticate, async (req, res) => {
    try {
        const post = await postModel.findById(req.params.id);
        if (!post) {
            return res.status(404).render('Edit', { errors: ["Post not found"], post: null });
        }
        if (post.author.toString() !== req.user._id.toString()) {
            return res.status(403).render('Edit', { errors: ["You are not authorized to edit this post"], post });
        }
        res.render('Edit', { post, errors: [] });
    } catch (error) {
        res.status(500).render('Edit', { errors: ["Error fetching post"], post: null });
    }
});
...
router.post('/editpost/:id', authenticate, upload.single("image"), [
    ...
    body('image').custom((value, { req }) => {
        if (req.file) {
            ...
            const validTypes = ['image/jpeg', 'image/png', 'image/webp'];
            if (fileSize < 1 || fileSize > 6) {
                throw new Error('Image size must be between 0-6 MB');
            }
            if (!validTypes.includes(fileType)) {
                throw new Error('Invalid file type. Only jpg, jpeg, webp, and png are allowed');
            }
    ...
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        console.log('Errors:', errors.array()); // Log errors
